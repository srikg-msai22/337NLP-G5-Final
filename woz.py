...
def make_woz_datasets(bKnowledge):
    if bKnowledge:
        ...
        out_names = ['woz.train_b.txt', 'woz.valid_a.txt', 'woz.test_a.txt', 'woz.valid_b.txt',
                     'woz.test_b.txt']
    max_ins = [18, 2, 2, 2, 2]
...
def main():
    ...
    with open(test_name, 'rt') as f:
        for line in f:
            ...
            if gen_mode == 2:
                input_ids = torch.tensor(in_ids).unsqueeze(0)
                input_ids = input_ids.cuda()
                beam = model.generate(input_ids, max_length=max_len, num_beams=5,
                                      early_stopping=True)
            ...
            if gen_mode == 3:
                input_ids = torch.tensor(in_ids).unsqueeze(0)
                input_ids = input_ids.cuda()
                top_p = model.generate(input_ids, max_length=max_len, do_sample=True, top_p=0.90,
                                       top_k=0)
    ...
    print('Final %s on %s BLEU: %7.3f % 7.3f' % (
    gen_labels[gen_mode], test_name, results['bleu'], sum(bleus) / 511.0))
